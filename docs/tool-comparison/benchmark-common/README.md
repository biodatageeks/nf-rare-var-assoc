...
| `three_method_ap_bar.py` | Grouped bar chart of recall-scaled average precision, one group per dataset and one bar per method. The three arms are set with `--arm-a` / `--arm-b` / `--arm-c`, each taking a label and an eval subdirectory, as `pairwise_compare.py` does; colour and left-to-right position belong to the slot, so a method looks the same across figures. `--style panel` sizes it for the printed page. |
