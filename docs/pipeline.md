...
- **Normalisation against the reference genome.** Indels are left-aligned and trimmed to
  ...
  convention, so normalising is what lets variants be matched against them reliably. The
  reference used is the FASTA given with `--input_ref_fasta`, or, when that is not set,
  one downloaded by `nf-prepare-vcf` -- GRCh38 by default. The same reference is then
  used for VEP annotation.
