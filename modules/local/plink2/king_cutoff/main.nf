process PLINK2_KING_CUTOFF {
    ...
    val(king_cutoff_threshold)
    val(out_name_part)
    val(input_args)
    path(tracking_in)

    output:
    tuple val(meta), path("*.king.cutoff.in.id"), emit: out_prune_in
    tuple val(meta), path("*.king.cutoff.out.id"), emit: out_prune_out
    ...
    plink2 \\
        ...
        ${psam_input} \\
        ${extract_input} \\
        --king-cutoff ${king_cutoff_threshold} \\
