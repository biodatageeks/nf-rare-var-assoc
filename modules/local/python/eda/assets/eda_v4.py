#!/usr/bin/env python3
"""Exploratory data analysis for a multisample VCF (v4).

This is a direct refactor of eda__old.py to reduce peak memory usage by
loading VCF rows in chunks and extending a Polars DataFrame incrementally.
"""
import os
...
def _extend_df(vcf_df, data, dtypes):
    if not data['CHROM']:
        return vcf_df
    chunk_df = pl.DataFrame(data, schema=dtypes)
    if vcf_df is None:
        return chunk_df
    vcf_df.extend(chunk_df)
    return vcf_df

# Function to load VCF.gz and extract DP, GQ, and missingness
def load_vcf(vcf_file, chunk_size=1000):
    print(f"load_vcf({vcf_file})  ts = {current_milli_time()}", flush=True)
    vcf = pysam.VariantFile(vcf_file)
    samples = list(vcf.header.samples)

    dtypes = {'CHROM': pl.Utf8, 'POS': pl.Int32, 'Variant_Type': pl.Int8, 'AF': pl.Float16}
    for sample in samples:
        dtypes[f'DP_{sample}'] = pl.Float32
        dtypes[f'GQ_{sample}'] = pl.Float32
        dtypes[f'GT_{sample}'] = pl.Int8
        dtypes[f'DS_{sample}'] = pl.Float32

    vcf_df = None
    data = _init_chunk(samples)
    ...
    for record in vcf.fetch():
        ...
        if i % chunk_size == 0:
            vcf_df = _extend_df(vcf_df, data, dtypes)
            data = _init_chunk(samples)
    ...
    if vcf_df is None:
        vcf_df = pl.DataFrame(schema=dtypes)
