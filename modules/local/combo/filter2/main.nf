// println "Workflow object: ${workflow}"
// println "Container engine: ${workflow?.containerEngine}"
process BCFTOOLS_FILTER_2_TIMES {
    tag "$meta.id"
    ...
    script:
    def args1 = (task.ext.args ?: '') + ' ' + input_args_1
    def args2 = (task.ext.args ?: '') + ' ' + input_args_2
    ...
    def extension1 = args1.contains("--output-type b") || args1.contains("-Ob") ? "bcf.gz" :
                    args1.contains("--output-type u") || args1.contains("-Ou") ? "bcf" :
                    args1.contains("--output-type z") || args1.contains("-Oz") ? "vcf.gz" :
                    args1.contains("--output-type v") || args1.contains("-Ov") ? "vcf" :
                    "vcf"
    def extension2 = args2.contains("--output-type b") || args2.contains("-Ob") ? "bcf.gz" :
    ...
    bcftools filter \\
        --output filter_1_out.${extension1} \\
        ${regions_file} \\
        ${targets_file} \\
        ${samples_file} \\
        ${snplist_file} \\
    ...
    bcftools filter \\
        --output ${prefix}_${out_name_part}.${extension2} \\
        ${regions_file} \\
        ${targets_file} \\
        ${samples_file} \\
        ${snplist_file} \\
        $args2 \\
        --threads $task.cpus \\
        filter_1_out.${extension1}
    
    rm filter_1_out.${extension1}

    bcftools stats ${vcf} > ${prefix}_${out_name_part}_${extension2}_in_stats.txt
    bcftools stats ${prefix}_${out_name_part}.${extension2} > ${prefix}_${out_name_part}_${extension2}_out_stats.txt


    # Extract counts from stats files
    samples_in=\$(grep "number of samples:" ${prefix}_${out_name_part}_${extension2}_in_stats.txt | cut -f4)
    variants_in=\$(grep "number of records:" ${prefix}_${out_name_part}_${extension2}_in_stats.txt | cut -f4)
    samples_out=\$(grep "number of samples:" ${prefix}_${out_name_part}_${extension2}_out_stats.txt | cut -f4)
    variants_out=\$(grep "number of records:" ${prefix}_${out_name_part}_${extension2}_out_stats.txt | cut -f4)
    ...
    cat <<-END_TRACKING_JSON > \$out_tracking_file_name
    {
        ...
        "parameters": "${regions_file} ${targets_file} ${samples_file} ${snplist_file}  $args1  |  $args2",
